...
public class LoginTest extends BaseTest {
    ...
    public void shouldReturn422StatusCodeWithErrorMessageWhenLoginWithIncorrectData() {
        ...
        SoftAssertions.assertSoftly(softly -> {
            ...
            softly.assertThat(jsonPath.getString("message")).isNotBlank().containsIgnoringCase("Вы ввели неверный номер телефона и/или пароль");
        });
    ...
    public void shouldReturn500StatusCodeWhenLoginWithEmptyEmailAndEmptyPassword() {
        ...
        String responseBody = service.getBody().trim();
        JsonPath jsonPath = new JsonPath(service.getBody());
    ...
    public void shouldReturn422StatusCodeWhenLoginWithValidPhoneAndEmptyPassword() {
        ...
        try {
            ...
            JsonNode phoneErrors = errorsNode.get("phone");

            SoftAssertions.assertSoftly(softly -> {
                softly.assertThat(responseBody).isNotNull().isNotBlank();
                softly.assertThat(service.getStatusCode()).isEqualTo(UNPROCESSABLE_ENTITY_CODE);
                softly.assertThat(jsonResponse.get("status").asText()).isEqualTo("error");
                softly.assertThat(jsonResponse.get("message").asText()).isEqualTo("Вы ввели неверный номер телефона и/или пароль");
                softly.assertThat(phoneErrors.isArray()).isTrue();
                softly.assertThat(phoneErrors.size()).isEqualTo(1);
                softly.assertThat(phoneErrors.get(0).asText()).isEqualTo("Вы ввели неверный номер телефона и/или пароль");
            });
        } catch (Exception e) {
            Assertions.fail("Failed to parse response JSON: " + e.getMessage());
        }
    }
    ...
    public void shouldReturn422StatusCodeWithErrorMessageWhenLoginWithNonNumericPhone() {
        ...
        try {
            ...
            SoftAssertions.assertSoftly(softly -> {
                ...
                softly.assertThat(errorsNode).isNotNull().isNotEmpty();
                softly.assertThat(phoneErrors.isArray()).isTrue();
                softly.assertThat(phoneErrors.get(0).asText()).isEqualTo("Вы ввели неверный номер телефона и/или пароль");
            });
        } catch (Exception e) {
    ...
    public void shouldReturn422StatusCodeWithErrorMessageWhenPhoneHavingFewerThan12Digits() {
        ...
        try {
            ...
            SoftAssertions.assertSoftly(softly -> {
                ...
                softly.assertThat(jsonResponse.get("status").asText()).isEqualTo("error");
                softly.assertThat(jsonResponse.get("message").asText()).isEqualTo("Вы ввели неверный номер телефона и/или пароль");
                softly.assertThat(errorsNode).isNotEmpty().isNotNull();
                softly.assertThat(errorsNode.has("phone")).isTrue();
    ...
    public void shouldReturn422StatusCodeWithErrorMessageWhenPhoneHavingMoreThan12Digits() {
        ...
        SoftAssertions.assertSoftly(softly -> {
            ...
            softly.assertThat(phoneErrors.getFirst()).isEqualTo("Вы ввели неверный номер телефона и/или пароль");
        });
    }
}
